...
PROOFREADER NOTE: I find this Figure incredibly confusing with all of the lines. It's also not very clear that the black lines are triangles. Maybe you could change it. I'd suggest having only 2 triangles in the Figure, and drawing the full triangle, not just the black line.
...
<details>
Below we use a different approach to explain the depth interpolation issue (but we prefer the one above). You can see the triangle (in 3D or camera space) lying on a plane. The plane equation is (equation 1):
